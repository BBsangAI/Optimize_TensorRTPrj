...
bool Model::build_engine() {
    ...
    if(builder->platformHasFastInt8() && m_params->prec == model::INT8){
            ...
            if(m_params->cal == mdoel::calibrator::Entropy){
    ...
    profile->setDimensions(input->getName(), nvinfer1::OptProfileSelector::kMIN, nvinfer1::Dims5(1, 3, 16, 112, 112)); //设置最小尺寸
    profile->setDimensions(input->getName(), nvinfer1::OptProfileSelector::kOPT, nvinfer1::Dims5(1, 3, 16, 112, 112)); //设置优化尺寸
    profile->setDimensions(input->getName(), nvinfer1::OptProfileSelector::kMAX, nvinfer1::Dims5(1, 3, 16, 112, 112)); //设置最大尺寸
